...
/*
 * Recuperación de contraseña (envío de correo simulado)
 * POST { email }                    -> genera un código de 6 dígitos
 * POST { email, codigo, password }  -> valida el código y cambia la contraseña
 * Los códigos viven en memoria y expiran a los 10 minutos
 */
...
export const POST = manejar(async (request) => {
  ...
  const usuario = await repoUsuarios.porEmail(email);
  const codigos = almacenCodigos();

  // Función para solicitar el código
  if (esTextoVacio(body.codigo)) {
    ...
    return ok({
      enviado: true,
      mensaje: 'Si el correo está registrado, recibirás un código de verificación.',
      codigoDemo: codigo, // solo por ser un entorno de demostración
    });
  }

  // Función para confirmar el código y cambiar la contraseña
  ...
  if (usuario) await repoUsuarios.cambiarPassword(email, body.password);
  codigos.delete(email);
